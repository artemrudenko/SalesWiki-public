...
Demo-роли (задаются через `SALESWIKI_DEMO_ACTOR`): `demo-ivan-ae` (аккаунт-менеджер),
`demo-nina-marketing` (маркетинг), `demo-elena-hos` (руководитель продаж),
`demo-marina-curator` (куратор/аппрувер), `demo-broad-viewer` (наблюдатель).

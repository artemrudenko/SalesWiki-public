...
class GovernanceInbox(unittest.TestCase):
    ...
    def test_reviewers_see_queue(self) -> None:
        for who in ("demo-sophie-curator", "demo-claire-hos", "demo-raj-revops", "demo-ada-admin"):
    ...
    def test_non_reviewers_are_blocked_from_queue(self) -> None:
        for who in ("demo-broad-viewer", "demo-sam-sdr", "demo-olivia-marketing", "demo-ethan-ae", "demo-hannah-legal"):
            out = self.svc.review_queue(actor(who))
    ...
    def test_queue_returns_server_decision_capability(self) -> None:
        for who in ("demo-sophie-curator", "demo-claire-hos", "demo-ada-admin"):
    ...
    def test_get_proposal_for_reviewer(self) -> None:
        out = self.svc.get_proposal(actor("demo-sophie-curator"), self.pid)
        self.assertEqual(out["status"], "draft")
        self.assertEqual(out["proposal"]["requester"], "demo-ethan-ae")

    def test_get_proposal_unknown_is_safe(self) -> None:
        out = self.svc.get_proposal(actor("demo-sophie-curator"), "proposal-9999")
        self.assertEqual(out["status"], "not-found")

    def test_get_proposal_blocked_for_non_reviewer(self) -> None:
        out = self.svc.get_proposal(actor("demo-olivia-marketing"), self.pid)
    ...
    def test_curator_can_reject_append_only_no_mutation(self) -> None:
        ...
        result = self.svc.reject_proposal(actor("demo-sophie-curator"), self.pid, "not a real issue")
    ...
    def test_rejected_proposal_is_never_applied(self) -> None:
        self.svc.reject_proposal(actor("demo-sophie-curator"), self.pid, "no")
    ...
    def test_audit_records_governance(self) -> None:
        self.svc.review_queue(actor("demo-sophie-curator"))
        self.svc.reject_proposal(actor("demo-sophie-curator"), self.pid, "no")
